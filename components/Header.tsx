      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", justifyContent: "flex-end", minWidth: 0 }}>
          {account ? (
            <>
              {!chainOk && (
                <button onClick={() => switchToArc().catch(() => {})} className="btn" style={{ borderColor: "rgba(255,90,77,0.5)", color: "var(--bad)", padding: "9px 13px", fontSize: 13 }}>
                  Wrong network
                </button>
              )}
              <div style={{ position: "relative" }}>
                <button
                  onClick={() => setOpen((o) => !o)}
                  className="chip"
                  style={{ padding: "9px 13px", cursor: "pointer", color: "var(--ink)" }}
                >
                  <span className="dot" style={{ background: chainOk ? "var(--volt)" : "var(--bad)" }} />
                ...
                {open && (
                  <>
                    ...
                    <div className="panel" style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, zIndex: 61, minWidth: 232, overflow: "hidden", boxShadow: "0 18px 50px -16px rgba(0,0,0,0.7)" }}>
